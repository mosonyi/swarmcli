...
- **App bootstrap:** [app/app.go](app/app.go) initializes logging and triggers autoload via blank imports — commands (`_ "github.com/Eldara-Tech/swarmcli/v2/commands"`) and views (`_ "github.com/Eldara-Tech/swarmcli/v2/views"`). Views self-register with the `view.Factory` pattern; the registry lives in [views/view/registry.go](views/view/registry.go).
